                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                ...
                                <div class="flex flex-col space-y-2">
                                    ...
                                    <select id="sortBy" class="form-select bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-600 dark:text-gray-300 rounded-lg px-4 py-2.5 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200 hover:border-green-300 cursor-pointer">
                                        ...
                                        <option value="membership_type_asc">Membership Type (A-Z)</option>
                                        <option value="membership_type_desc">Membership Type (Z-A)</option>
                                        <option value="bureau_asc">Bureau (A-Z)</option>
                                        <option value="bureau_desc">Bureau (Z-A)</option>
                                    </select>
                                </div>
                    ...
                    <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
                        <div class="min-w-[800px]">
                            <table id="unlicensedTable" class="w-full bg-white dark:bg-gray-900 text-sm">
                                <thead class="bg-[#101966] dark:bg-gray-700 text-gray-200 dark:text-gray-200">
                                    <tr class="border-b dark:border-gray-700">
                                        ...
                                        <th class="px-6 py-3 text-center font-medium border-l dark:border-gray-700 border-white column-membership_type">Membership Type</th>
                                        <th class="px-6 py-3 text-center font-medium border-l dark:border-gray-700 border-white column-bureau">Bureau</th>
                                        <th class="px-6 py-3 text-center font-medium border-l dark:border-gray-700 border-white column-section">Section</th>
    ...
    <x-slot name="script">
        ...
        <script>
            $(document).ready(function () {
                ...
                function fetchUnlicensedMembers(page = 1, search = '', perPage = $('#perPage').val() || $('#mobilePerPage').val()) {
                    ...
                    const sortMap = {
                        ...
                        'membership_type_asc': { sort: 'membership_type', direction: 'asc' },
                        'membership_type_desc': { sort: 'membership_type', direction: 'desc' },
                        'bureau_asc': { sort: 'bureau', direction: 'asc' },
                        'bureau_desc': { sort: 'bureau', direction: 'desc' },
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            renderUnlicensedMembers(response.data, response.from);
                ...
                function renderUnlicensedMembers(members, startIndex) {
                    let tbody = $('#unlicensedTable tbody');
                    tbody.empty();
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-lg font-medium mb-2">No unlicensed members found</p>
                                        <p class="text-sm">There are no unlicensed members matching your search criteria.</p>
                                    </div>
                                </td>
                            </tr>
                        `);
                    ...
                    members.forEach((member, index) => {
                        ...
                        let row = `
                            <tr class="border-b table-row-hover table-row-animate dark:border-gray-700">
                                ...
                                <td class="px-6 py-4 text-center column-name">${member.name}</td>
                                <td class="px-6 py-4 text-center column-membership_type">${member.membership_type}</td>
                                <td class="px-6 py-4 text-center column-bureau">${member.bureau}</td>
                                <td class="px-6 py-4 text-center column-section">${member.section}</td>
                ...
                function renderPagination(data) {
                    ...
                    if (data.current_page > 1) {
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchUnlicensedMembers(1, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                &laquo; First
                            </button>
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchUnlicensedMembers(${data.current_page - 1}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                Previous
                            </button>`;
                    }
                    ...
                    for (let i = startPage; i <= endPage; i++) {
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border ${i === currentPage ? 'bg-[#101966] text-white' : 'bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white'}"
                                onclick="fetchUnlicensedMembers(${i}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                ${i}
                            </button>`;
                    }
                    if (endPage < totalPages) {
                        ...
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border ${totalPages === currentPage ? 'bg-[#101966] text-white' : 'bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white'}"
                                onclick="fetchUnlicensedMembers(${totalPages}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                ${totalPages}
                            </button>`;
                    }
                    if (data.current_page < data.last_page) {
                        paginationHtml += `
                            ...
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchUnlicensedMembers(${data.last_page}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                Last &raquo;
                            </button>`;
                    }
                    paginationHtml += '</div>';
                ...
                window.fetchUnlicensedMembers = fetchUnlicensedMembers;
            });
        </script>
    </x-slot>
</x-app-layout>