<x-app-layout>
    <x-slot name="header">
        <x-page-header title="{{ __('Marquee') }}">
            <x-slot name="createButton">
                <x-create-button 
                    :route="route('markees.create')" 
                    permission="create markees" />
            </x-slot>
        </x-page-header>
    </x-slot>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <div class="relative w-48">
                                ...
                                <input type="text" id="searchInput" 
                                    placeholder="Search Header" class="pl-8 pr-2 py-2 border border-gray-300 dark:border-gray-600 
                    ...
                    <div class="sm:hidden space-y-3 mb-4">
                        <div class="relative w-full">
                            ...
                            <input type="text" id="mobileSearchInput" placeholder="Search Header/Content" 
                        ...
                        <div id="mobileResultInfo" class="text-sm text-gray-700 dark:text-gray-300 text-center">
                            Showing <span id="mobileStartRecord">0</span> to <span id="mobileEndRecord">0</span> of <span id="mobileTotalRecords">0</span> markees
                        </div>
                    </div>
                            ...
                            <table id="markeesTable" class="w-full bg-white dark:bg-gray-900 text-sm">
                                <thead class="bg-[#101966] dark:bg-gray-700 text-gray-200 dark:text-gray-200">
                                    <tr class="border-b dark:border-gray-700">
                                        ...
                                        <th class="px-6 py-3 text-center font-medium border-l dark:border-gray-700 border-white column-header">Header</th>
        ...
        <script>
            $(document).ready(function () {
                ...
                $('#perPage, #mobilePerPage').on('change', function () {
                    fetchMarkees(1, $('#searchInput').val() || $('#mobileSearchInput').val(), $(this).val());
                });

                $('#sortBy, #mobileSortBy').on('change', function() {
                    fetchMarkees(1, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val());
                });

                function fetchMarkees(page = 1, search = '', perPage = $('#perPage').val() || $('#mobilePerPage').val()) {
                    const sortValue = $('#sortBy').val() || $('#mobileSortBy').val() || 'created_desc';
                    const [column, direction] = sortValue.split('_');

                    const sortMap = {
                        'header_asc': { sort: 'header', direction: 'asc' },
                        'header_desc': { sort: 'header', direction: 'desc' },
                        ...
                        tbody.append(`
                            <tr class="border-b dark:border-gray-700">
                                <td colspan="7" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                                    No markees found matching your search criteria
                                </td>
                            </tr>
                        `);
                        return;
                    ...
                    markees.forEach((markee, index) => {
                        ...
                        tbody.append(`
                            <tr class="border-b table-row-hover table-row-animate dark:border-gray-700">
                                ...
                                <td class="px-6 py-4 text-center column-header">${markee.header}</td>
                                <td class="px-6 py-4 text-center column-content">${markee.content}</td>
                                <td class="px-6 py-4 text-center column-author">${markee.author}</td>
                                ...
                                <td class="px-6 py-4 text-center column-created">${markee.created_at}</td>
                                <td class="px-6 py-4 text-center">
                                    <div class="flex justify-center items-center space-x-2">
                                        <a href="/markees/${markee.id}/edit" 
                                        ...
                                        <button onclick="deleteMarkee(${markee.id})" 
                ...
                function renderPagination(data) {
                    ...
                    if (data.current_page > 1) {
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchMarkees(1, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                &laquo; First
                            </button>
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchMarkees(${data.current_page - 1}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                Previous
                            </button>`;
                    }
                    ...
                    for (let i = startPage; i <= endPage; i++) {
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border ${i === currentPage ? 'bg-[#101966] text-white' : 'bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white'}"
                                onclick="fetchMarkees(${i}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                ${i}
                            </button>`;
                    }
                    ...
                    if (endPage < totalPages) {
                        ...
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border ${totalPages === currentPage ? 'bg-[#101966] text-white' : 'bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white'}"
                                onclick="fetchMarkees(${totalPages}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                ${totalPages}
                            </button>`;
                    }
                    ...
                    if (data.current_page < data.last_page) {
                        paginationHtml += `
                            ...
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchMarkees(${data.last_page}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                Last &raquo;
                            </button>`;
                    }
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                ...
                                success: function (response) {
                                    Swal.fire({
                                        title: 'Deleted!',
                                        text: 'Marquee item has been deleted successfully.',
                                        icon: 'success',
                                        background: '#101966',
                                        color: '#fff'
                                    });
                                ...
                                error: function() {
                                    Swal.fire({
                                        title: 'Error!',
                                        text: 'Something went wrong while deleting the marquee item.',
                                        icon: 'error',
                                        background: '#101966',
                                        color: '#fff'
                                    });
