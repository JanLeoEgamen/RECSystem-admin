                    ...
                    <div class="mt-6 sm:mt-8 flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
                        ...
                        <div class="flex-1 bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-500 p-3 sm:p-4 rounded-r-lg">
                            <div class="flex items-start sm:items-center gap-2 sm:gap-3">
                                ...
                                <p class="text-xs sm:text-sm text-blue-800 dark:text-blue-200 font-medium">Review this applicant's information. You can edit, delete, or assess their application.</p>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col sm:flex-row gap-3 sm:flex-nowrap">
                            ...
                            <form id="deleteForm" action="{{ route('applicants.destroy') }}" method="POST" class="inline w-full sm:w-auto">
                                @csrf
                                @method('DELETE')
                                <input type="hidden" name="id" value="{{ $applicant->id }}">
                                <button type="submit" class="inline-flex items-center justify-center px-6 sm:px-8 py-2.5 sm:py-3 text-white bg-gradient-to-r from-red-600 to-pink-600 
                                        ...
                                        transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105 w-full">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 sm:h-6 sm:w-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            @if($applicant->status === 'rejected')
                                @can('edit applicants')
                                <form action="{{ route('applicants.restore', $applicant->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to restore this applicant to pending status?');" class="inline w-full sm:w-auto">
                                    @csrf
                                    <button type="submit" class="inline-flex items-center justify-center px-6 sm:px-8 py-2.5 sm:py-3 text-white bg-gradient-to-r from-green-600 to-emerald-600 
                                            hover:from-green-700 hover:to-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 
                                            focus:ring-green-600 border-2 border-transparent font-bold rounded-xl text-base sm:text-lg 
                                            transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105 w-full">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 sm:h-6 sm:w-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            @if($applicant->status === 'pending')
                                ...
                                <a href="{{ route('applicants.assess', $applicant->id) }}" class="inline-flex items-center justify-center px-6 sm:px-8 py-2.5 sm:py-3 text-white bg-gradient-to-r from-blue-600 to-cyan-600 
