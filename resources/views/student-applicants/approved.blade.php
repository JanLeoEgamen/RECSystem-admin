    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <h2 class="font-semibold text-4xl text-white dark:text-gray-200 leading-tight text-center sm:text-left">
                {{ __('Approved Student Applicants') }}
            </h2>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
                        <div class="min-w-[1000px]">
                            <table id="approvedStudentApplicantsTable" class="w-full bg-white dark:bg-gray-900 text-sm">
    ...
    <x-slot name="script">
        ...
        <script>
            $(document).ready(function () {
                fetchApprovedStudentApplicants();

                $('#searchInput, #mobileSearchInput').on('keyup', function () {
                    fetchApprovedStudentApplicants(1, $(this).val());
                });

                $('#perPage, #mobilePerPage').on('change', function () {
                    fetchApprovedStudentApplicants(1, $('#searchInput').val() || $('#mobileSearchInput').val(), $(this).val());
                });

                $('#sortBy, #mobileSortBy').on('change', function() {
                    fetchApprovedStudentApplicants(1, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val());
                });

                function fetchApprovedStudentApplicants(page = 1, search = '', perPage = $('#perPage').val() || $('#mobilePerPage').val()) {
                    ...
                    $.ajax({
                        url: `{{ route('student-applicants.approved') }}`,
                        type: 'GET',
                        data: {
                            page: page,
                            search: search,
                        ...
                        success: function (response) {
                            renderApprovedStudentApplicants(response.data, response.from);
                ...
                function renderApprovedStudentApplicants(applicants, startIndex) {
                    let tbody = $('#approvedStudentApplicantsTable tbody');
                    tbody.empty();
                            ...
                            <tr>
                                <td colspan="8" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                    <div class="flex flex-col items-center justify-center space-y-4">
                                        ...
                                        <div class="text-lg font-medium">No approved student applicants found</div>
                                        <div class="text-sm">There are currently no approved student applicants to display.</div>
                                    </div>
                                </td>
                            </tr>
                        `);
                ...
                function renderPagination(data) {
                    ...
                    if (data.current_page > 1) {
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchApprovedStudentApplicants(1, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                &laquo; First
                            </button>
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchApprovedStudentApplicants(${data.current_page - 1}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                Previous
                            </button>`;
                    }
                    ...
                    for (let i = startPage; i <= endPage; i++) {
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border ${i === currentPage ? 'bg-[#101966] text-white' : 'bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white'}"
                                onclick="fetchApprovedStudentApplicants(${i}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                ${i}
                            </button>`;
                    }
                    if (endPage < totalPages) {
                        ...
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border ${totalPages === currentPage ? 'bg-[#101966] text-white' : 'bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white'}"
                                onclick="fetchApprovedStudentApplicants(${totalPages}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                ${totalPages}
                            </button>`;
                    }
                    if (data.current_page < data.last_page) {
                        paginationHtml += `
                            ...
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchApprovedStudentApplicants(${data.last_page}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                Last &raquo;
                            </button>`;
                    }
                    paginationHtml += '</div>';
                ...
                window.fetchApprovedStudentApplicants = fetchApprovedStudentApplicants;
            });
        </script>
    </x-slot>
</x-app-layout>