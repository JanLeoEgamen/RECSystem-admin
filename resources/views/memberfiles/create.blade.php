        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form id="createMemberFileForm" action="{{ route('memberfiles.store') }}" method="post">
                        @csrf
                        <div>
                            ...
                            <div class="my-3">    
                                <textarea name="description" id="description" placeholder="Enter description" class="border-gray-300 shadow-sm w-full rounded-lg" rows="3">{{ old('description') }}</textarea>
                                @error('description')
                                <p class="text-red-400 font-medium"> {{ $message }} </p>
                                @enderror
                            </div>
                            ...
                            <div class="my-3">
                                <div class="relative">
                                    <select name="member_ids[]" id="members-select" multiple class="hidden">
                                        @foreach($members as $member)
                                            <option value="{{ $member->id }}" {{ in_array($member->id, old('member_ids', [])) ? 'selected' : '' }}>
                                                {{ $member->first_name }} {{ $member->last_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <div id="members-dropdown" class="w-1/2 mt-1">
                                        <div class="relative">
                                        ...
                                        <div id="selected-members" class="mt-2 flex flex-wrap gap-2">
                                            @foreach($members as $member)
                                                @if(in_array($member->id, old('member_ids', [])))
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div class="my-3">    
                                        <input value="{{ old('due_date') }}" name="due_date" id="due_date" type="date" class="border-gray-300 shadow-sm w-full rounded-lg">
                                        @error('due_date')
                                        <p class="text-red-400 font-medium"> {{ $message }} </p>
                                        @enderror
                                    </div>
                                ...
                                <div>
                                    ...
                                    <div class="my-3 flex items-center">    
                                        ...
                                        <input type="checkbox" name="is_required" id="is_required" class="rounded" value="1" {{ old('is_required', false) ? 'checked' : '' }}>
    ...
    <x-slot name="script">
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.getElementById('createMemberFileForm').addEventListener('submit', function(e) {
                    ...
                    Swal.fire({
                        title: "Are you sure?",
                        text: "Do you want to create this file assignment for " + selectedCount + " member(s)?",
                        icon: "warning",
