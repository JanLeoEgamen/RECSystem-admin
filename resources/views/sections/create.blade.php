<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            <h2 class="font-semibold text-4xl text-white dark:text-gray-200 leading-tight text-center md:text-left">
                Sections <span class="block md:hidden">/ Create</span>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form id="createForm" action="{{ route('sections.store') }}" method="post">
                        @csrf
                        <div>
                            <label for="section_name" class="text-sm font-medium">Section Name</label>
                            <div class="my-3">    
                                <input value="{{ old('section_name') }}" name="section_name" placeholder="Enter section name" type="text" class="border-gray-300 shadow-sm w-1/2 rounded-lg">
                                @error('section_name')
    ...
    <script>
        document.getElementById("createForm").addEventListener("submit", function(e) {
            ...
            Swal.fire({
                title: "Are you sure?",
                text: "Do you want to create this section?",
                icon: "warning",
