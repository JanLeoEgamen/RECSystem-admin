<x-app-layout>
    <x-slot name="header">
        <x-page-header title="{{ __('Sections') }}">
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <select id="sortBy" class="form-select border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 rounded px-4 py-1 pr-10 text-sm focus:outline-none focus:ring focus:border-blue-300 w-48">
                                    ...
                                    <option value="section_name_asc">Name (A-Z)</option>
                                    <option value="section_name_desc">Name (Z-A)</option>
                                    <option value="bureau_name_asc">Bureau (A-Z)</option>
                                    <option value="bureau_name_desc">Bureau (Z-A)</option>
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <div class="relative w-48">
                                ...
                                <input type="text" id="searchInput" 
                                    placeholder="Search Section Name" class="pl-8 pr-2 py-2 border border-gray-300 dark:border-gray-600 
                    ...
                    <div class="sm:hidden space-y-3 mb-4">
                        <div class="relative w-full">
                            ...
                            <input type="text" id="mobileSearchInput" placeholder="Search Section Name" 
                    ...
                    <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
                        <div class="min-w-[600px]">
                            <table id="sectionsTable" class="w-full bg-white dark:bg-gray-900 text-sm">
                                <thead class="bg-[#101966] dark:bg-gray-700 text-gray-200 dark:text-gray-200">
                                    <tr class="border-b dark:border-gray-700">
                                        ...
                                        <th class="px-6 py-3 text-center font-medium border-l dark:border-gray-700 border-white column-section_name">Section Name</th>
                                        <th class="px-6 py-3 text-center font-medium border-l dark:border-gray-700 border-white column-bureau_name">Bureau</th>
    ...
    <x-slot name="script">
        ...
        <script>
            $(document).ready(function () {
                ...
                function fetchSections(page = 1, search = '', perPage = $('#perPage').val() || $('#mobilePerPage').val()) {
                ...
                function renderSections(sections, startIndex) {
                    let tbody = $('#sectionsTable tbody');
                    tbody.empty();
                    ...
                    sections.forEach((section, index) => {
                        ...
                        tbody.append(`
                            <tr class="border-b table-row-hover table-row-animate dark:border-gray-700">
                                ...
                                <td class="px-6 py-4 text-center column-section_name">${section.section_name}</td>
                                <td class="px-6 py-4 text-center column-bureau_name">${section.bureau_name}</td>
                                <td class="px-6 py-4 text-center column-created">${section.created_at}</td>
                ...
                function renderPagination(data) {
                    ...
                    if (data.current_page > 1) {
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchSections(1, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                &laquo; First
                            </button>
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchSections(${data.current_page - 1}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                Previous
                            </button>`;
                    }
                    ...
                    for (let i = startPage; i <= endPage; i++) {
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border ${i === currentPage ? 'bg-[#101966] text-white' : 'bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white'}"
                                onclick="fetchSections(${i}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                ${i}
                            </button>`;
                    }
                    if (endPage < totalPages) {
                        ...
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border ${totalPages === currentPage ? 'bg-[#101966] text-white' : 'bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white'}"
                                onclick="fetchSections(${totalPages}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                ${totalPages}
                            </button>`;
                    }
                    if (data.current_page < data.last_page) {
                        paginationHtml += `
                            ...
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchSections(${data.last_page}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                Last &raquo;
                            </button>`;
                    }
                    paginationHtml += '</div>';
