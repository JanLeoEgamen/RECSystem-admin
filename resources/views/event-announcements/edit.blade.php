                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form id="eventEditForm" action="{{ route('event-announcements.update', $eventAnnouncement->id) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div>
                            ...
                            <div class="my-3">    
                                <input id="event_name" value="{{ old('event_name', $eventAnnouncement->event_name) }}" name="event_name" placeholder="Enter event name" type="text" class="border-gray-300 shadow-sm w-1/2 rounded-lg">
                                @error('event_name')
                            ...
                            <div class="my-3">    
                                <input id="event_date" value="{{ old('event_date', $eventAnnouncement->event_date->format('Y-m-d')) }}" name="event_date" type="date" class="border-gray-300 shadow-sm w-1/2 rounded-lg">
                                @error('event_date')
                                <p class="text-red-400 font-medium"> {{ $message }} </p>
                                @enderror
                            </div>
                            ...
                            <div class="my-3">    
                                <input id="year" value="{{ old('year', $eventAnnouncement->year) }}" name="year" placeholder="Enter year" type="number" min="2000" max="2100" class="border-gray-300 shadow-sm w-1/2 rounded-lg">
                                @error('year')
                                <p class="text-red-400 font-medium"> {{ $message }} </p>
                                @enderror
                            </div>
                            ...
                            <div class="my-3">    
                                <textarea id="caption" name="caption" placeholder="Enter caption" class="border-gray-300 shadow-sm w-1/2 rounded-lg">{{ old('caption', $eventAnnouncement->caption) }}</textarea>
                                @error('caption')
                                <p class="text-red-400 font-medium"> {{ $message }} </p>
                                @enderror
                            </div>
