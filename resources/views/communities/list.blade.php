<x-app-layout>
    <x-slot name="header">
        <x-page-header title="{{ __('Community') }}">
            <x-slot name="createButton">
                <x-create-button 
                    :route="route('communities.create')" 
                    permission="create communities" />
            </x-slot>
        </x-page-header>
    </x-slot>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                ...
                                <div class="flex flex-col space-y-2">
                                    ...
                                    <select id="sortBy" class="form-select bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-600 dark:text-gray-300 rounded-lg px-4 py-2.5 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200 hover:border-green-300 cursor-pointer">
                                        ...
                                        <option value="content_asc">Content (A-Z)</option>
                                        <option value="content_desc">Content (Z-A)</option>
                    ...
                    <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
                        <div class="min-w-[1000px]">
                            <table id="communitiesTable" class="w-full bg-white dark:bg-gray-900 text-sm">
        ...
        <script>
            $(document).ready(function () {
                ...
                $('#searchInput, #mobileSearchInput').on('keyup', function () {
                    fetchCommunities(1, $(this).val());
                });

                $('#perPage, #mobilePerPage').on('change', function () {
                    fetchCommunities(1, $('#searchInput').val() || $('#mobileSearchInput').val(), $(this).val());
                });

                $('#sortBy, #mobileSortBy').on('change', function() {
                    fetchCommunities(1, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val());
                });

                function fetchCommunities(page = 1, search = '', perPage = $('#perPage').val() || $('#mobilePerPage').val()) {
                    const sortValue = $('#sortBy').val() || $('#mobileSortBy').val() || 'created_desc';
                    const [column, direction] = sortValue.split('_');

                    const sortMap = {
                        'content_asc': { sort: 'content', direction: 'asc' },
                        'content_desc': { sort: 'content', direction: 'desc' },
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            renderCommunities(response.data, response.from);
                ...
                function renderCommunities(communities, startIndex) {
                    let tbody = $('#communitiesTable tbody');
                    tbody.empty();
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-lg font-medium">No community posts found</p>
                                        <p class="text-sm mt-1">There are currently no community posts to display.</p>
                                    </div>
                                </td>
                            </tr>
                        `);
                    ...
                    communities.forEach((community, index) => {
                        ...
                        let row = `
                            <tr class="border-b table-row-hover table-row-animate dark:border-gray-700">
                                ...
                                <td class="px-6 py-4 text-left column-content">${community.content}</td>
                                <td class="px-6 py-4 text-center column-image">
                                    ${community.image_url 
                                        ? '<img src="'+community.image_url+'" alt="Community Image" class="h-20 w-20 object-cover rounded">'
                                        : 'No Image'}
                                </td>
                                <td class="px-6 py-4 text-center column-author">${community.author}</td>
                                ...
                                <td class="px-6 py-4 text-center column-created">${community.created_at}</td>
                ...
                function renderPagination(data) {
                    ...
                    if (data.current_page > 1) {
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchCommunities(1, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                &laquo; First
                            </button>
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchCommunities(${data.current_page - 1}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                Previous
                            </button>`;
                    }
                    ...
                    for (let i = startPage; i <= endPage; i++) {
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border ${i === currentPage ? 'bg-[#101966] text-white' : 'bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white'}"
                                onclick="fetchCommunities(${i}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                ${i}
                            </button>`;
                    }
                    if (endPage < totalPages) {
                        ...
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border ${totalPages === currentPage ? 'bg-[#101966] text-white' : 'bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white'}"
                                onclick="fetchCommunities(${totalPages}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                ${totalPages}
                            </button>`;
                    }
                    if (data.current_page < data.last_page) {
                        paginationHtml += `
                            ...
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchCommunities(${data.last_page}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                Last &raquo;
                            </button>`;
                    }
                    paginationHtml += '</div>';
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                ...
                                success: function (response) {
                                    Swal.fire({
                                        title: 'Deleted!',
                                        text: 'Community item has been deleted successfully.',
                                        icon: 'success',
                                        background: '#101966',
                                        color: '#fff'
                                    });
                                ...
                                error: function() {
                                    Swal.fire({
                                        title: 'Error!',
                                        text: 'Something went wrong while deleting the community item.',
                                        icon: 'error',
                                        background: '#101966',
                                        color: '#fff'
                                    });
