<x-app-layout>
    <x-slot name="header">
        <x-page-header title="{{ __('Database Backups') }}">
            ...
            <x-slot name="actionButton">
                ...
                <form action="{{ route('backups.cleanup') }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" class="bg-red-600 hover:bg-red-700 text-white font-medium py-2 px-4 rounded-md text-sm">
                        Cleanup Old Backups
                    </button>
                </form>
                @endcan
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="sm:hidden mb-6">
                        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-700 dark:to-gray-600 rounded-xl shadow-md border-2 border-blue-100 dark:border-gray-600 p-4 space-y-4">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative">
                                    <input type="text" id="mobileSearchInput" placeholder="Search Backups" 
                    ...
                    <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
                        <div class="min-w-[1000px]">
                            <table id="backupsTable" class="w-full bg-white dark:bg-gray-900 text-sm">
                                <thead class="bg-[#101966] dark:bg-gray-700 text-gray-200 dark:text-gray-200">
                                    <tr class="border-b dark:border-gray-700">
                                        ...
                                        <th class="px-6 py-3 text-center font-medium border-l dark:border-gray-700 border-white column-size">Size</th>
        ...
        <script>
            $(document).ready(function () {
                ...
                $('#searchInput, #mobileSearchInput').on('keyup', function () {
                    fetchBackups(1, $(this).val());
                });

                $('#perPage, #mobilePerPage').on('change', function () {
                    fetchBackups(1, $('#searchInput').val() || $('#mobileSearchInput').val(), $(this).val());
                });

                $('#sortBy, #mobileSortBy').on('change', function() {
                    fetchBackups(1, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val());
                });

                function fetchBackups(page = 1, search = '', perPage = $('#perPage').val() || $('#mobilePerPage').val()) {
                    ...
                    const sortMap = {
                        ...
                        'size_asc': { sort: 'size', direction: 'asc' },
                        'size_desc': { sort: 'size', direction: 'desc' },
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            renderBackups(response.data, response.from);
                ...
                function renderBackups(backups, startIndex) {
                    let tbody = $('#backupsTable tbody');
                    tbody.empty();
                            ...
                            <tr class="border-b dark:border-gray-700">
                                <td colspan="6" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                                    <div class="flex flex-col items-center space-y-3">
                                        ...
                                        <div class="text-lg font-medium text-gray-400 dark:text-gray-500">No backups found</div>
                                        <div class="text-sm text-gray-400 dark:text-gray-500">There are currently no database backups available to display</div>
                                    </div>
                                </td>
                            </tr>
                        `);
                    ...
                    backups.forEach((backup, index) => {
                        ...
                        tbody.append(`
                            <tr class="border-b table-row-hover table-row-animate dark:border-gray-700">
                                ...
                                <td class="px-6 py-4 text-left column-name">${backup.name}</td>
                                <td class="px-6 py-4 text-center column-size">${backup.size}</td>
                                <td class="px-6 py-4 text-center column-status">${backup.status}</td>
                                <td class="px-6 py-4 text-center column-created">${backup.created_at}</td>
                ...
                function renderPagination(data) {
                    ...
                    if (data.current_page > 1) {
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchBackups(1, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                &laquo; First
                            </button>
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchBackups(${data.current_page - 1}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                Previous
                            </button>`;
                    }
                    ...
                    for (let i = startPage; i <= endPage; i++) {
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border ${i === currentPage ? 'bg-[#101966] text-white' : 'bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white'}"
                                onclick="fetchBackups(${i}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                ${i}
                            </button>`;
                    }
                    ...
                    if (endPage < totalPages) {
                        ...
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border ${totalPages === currentPage ? 'bg-[#101966] text-white' : 'bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white'}"
                                onclick="fetchBackups(${totalPages}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                ${totalPages}
                            </button>`;
                    }
                    ...
                    if (data.current_page < data.last_page) {
                        paginationHtml += `
                            ...
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchBackups(${data.last_page}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                Last &raquo;
                            </button>`;
                    }
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                success: function (response) {
                    Swal.fire({
                        title: 'Deleted!',
                        text: 'Backup has been deleted successfully.',
                        icon: 'success',
                        background: '#101966',
                        color: '#fff'
                    });
                ...
                error: function(xhr) {
                    let errorMessage = 'Something went wrong while deleting the backup.';
