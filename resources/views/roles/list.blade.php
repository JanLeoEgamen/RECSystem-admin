                    ...
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <div class="flex items-center space-x-4 ml-6">
                            <div id="resultInfo" class="text-sm text-gray-700 dark:text-gray-300 whitespace-nowrap">
                                Showing <span id="startRecord">0</span> to <span id="endRecord">0</span> of <span id="totalRecords">0</span> roles
                            </div>
                            ...
                            <table id="rolesTable" class="w-full bg-white dark:bg-gray-900 text-sm">
                                <thead class="bg-[#101966] dark:bg-gray-800 text-gray-200 dark:text-gray-200">
                                    <tr class="border-b dark:border-gray-700">
                                        ...
                                        <th class="px-6 py-3 text-center font-medium border-l border-white column-permissions">Permissions</th>
        ...
        <script>
            $(document).ready(function () {
                ...
                $('#perPage').on('change', function () {
                    fetchRoles(1, $('#searchInput').val(), $(this).val());
                });

                $('#sortBy').on('change', function() {
                    fetchRoles(1, $('#searchInput').val(), $('#perPage').val());
                });
                ...
                function fetchRoles(page = 1, search = '', perPage = $('#perPage').val()) {
                    ...
                    $.ajax({
                        url: `{{ route('roles.index') }}?page=${page}&search=${search}&perPage=${perPage}${sortParams}`,
                        type: 'GET',
                ...
                function renderPagination(data) {
                    ...
                    if (data.current_page > 1) {
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200"
                                onclick="fetchRoles(1, $('#searchInput').val(), $('#perPage').val())">
                                &laquo; First
                            </button>
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200"
                                onclick="fetchRoles(${data.current_page - 1}, $('#searchInput').val(), $('#perPage').val())">
                                Previous
                            </button>`;
                    }
                    ...
                    for (let i = startPage; i <= endPage; i++) {
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border ${i === currentPage ? 'bg-[#101966] text-white' : 'bg-white hover:bg-gray-200'}"
                                onclick="fetchRoles(${i}, $('#searchInput').val(), $('#perPage').val())">
                                ${i}
                            </button>`;
                    }
                    ...
                    if (endPage < totalPages) {
                        ...
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border ${totalPages === currentPage ? 'bg-[#101966] text-white' : 'bg-white hover:bg-gray-200'}"
                                onclick="fetchRoles(${totalPages}, $('#searchInput').val(), $('#perPage').val())">
                                ${totalPages}
                            </button>`;
                    }
                    ...
                    if (data.current_page < data.last_page) {
                        paginationHtml += `
                            ...
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200"
                                onclick="fetchRoles(${data.last_page}, $('#searchInput').val(), $('#perPage').val())">
                                Last &raquo;
                            </button>`;
                    }
