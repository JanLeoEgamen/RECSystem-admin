<x-app-layout>
    <x-slot name="header">
        <x-page-header title="{{ __('Membership Types') }}">
            <x-slot name="createButton">
                <x-create-button 
                    :route="route('membership-types.create')" 
                    permission="create membership types" />
            </x-slot>
        </x-page-header>
    </x-slot>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
                        <div class="min-w-[600px]">
                            <table id="membershipTypesTable" class="w-full bg-white dark:bg-gray-900 text-sm">
                                <thead class="bg-[#101966] dark:bg-gray-700 text-gray-200 dark:text-gray-200">
                                    <tr class="border-b dark:border-gray-700">
                                        ...
                                        <th class="px-6 py-3 text-center font-medium border-l dark:border-gray-700 border-white column-type_name">Name</th>
        ...
        <script>
            $(document).ready(function () {
                ...
                $('#searchInput, #mobileSearchInput').on('keyup', function () {
                    fetchMembershipTypes(1, $(this).val());
                });

                $('#perPage, #mobilePerPage').on('change', function () {
                    fetchMembershipTypes(1, $('#searchInput').val() || $('#mobileSearchInput').val(), $(this).val());
                });

                $('#sortBy, #mobileSortBy').on('change', function() {
                    fetchMembershipTypes(1, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val());
                });

                function fetchMembershipTypes(page = 1, search = '', perPage = $('#perPage').val() || $('#mobilePerPage').val()) {
                    const sortValue = $('#sortBy').val() || $('#mobileSortBy').val() || 'created_desc';
                    const [column, direction] = sortValue.split('_');

                    const sortMap = {
                        'type_name_asc': { sort: 'type_name', direction: 'asc' },
                        'type_name_desc': { sort: 'type_name', direction: 'desc' },
                    ...
                    $.ajax({
                        url: `{{ route('membership-types.index') }}`,
                        type: 'GET',
                        data: {
                            page: page,
                            search: search,
                        ...
                        success: function (response) {
                            renderMembershipTypes(response.data, response.from);
                ...
                function renderMembershipTypes(membershipTypes, startIndex) {
                    let tbody = $('#membershipTypesTable tbody');
                    tbody.empty();
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-lg font-medium">No membership types found</p>
                                        <p class="text-sm mt-1">There are currently no membership types to display.</p>
                                    </div>
                                </td>
                            </tr>
                        `);
                    ...
                    membershipTypes.forEach((type, index) => {
                        ...
                        let row = `
                            <tr class="border-b table-row-hover table-row-animate dark:border-gray-700">
                                ...
                                <td class="px-6 py-4 text-center column-type_name">${type.type_name}</td>
                                <td class="px-6 py-4 text-center column-created">${type.created_at}</td>
                                <td class="px-6 py-4 text-center">
                                    <div class="flex justify-center items-center space-x-2">
                                        <a href="/membership-types/${type.id}/edit" 
                                        ...
                                        <button onclick="deleteMembershipType(${type.id})" 
                ...
                function renderPagination(data) {
                    ...
                    if (data.current_page > 1) {
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchMembershipTypes(1, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                &laquo; First
                            </button>
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchMembershipTypes(${data.current_page - 1}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                Previous
                            </button>`;
                    }
                    ...
                    for (let i = startPage; i <= endPage; i++) {
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border ${i === currentPage ? 'bg-[#101966] text-white' : 'bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white'}"
                                onclick="fetchMembershipTypes(${i}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                ${i}
                            </button>`;
                    }
                    if (endPage < totalPages) {
                        ...
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border ${totalPages === currentPage ? 'bg-[#101966] text-white' : 'bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white'}"
                                onclick="fetchMembershipTypes(${totalPages}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                ${totalPages}
                            </button>`;
                    }
                    if (data.current_page < data.last_page) {
                        paginationHtml += `
                            ...
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchMembershipTypes(${data.last_page}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                Last &raquo;
                            </button>`;
                    }
                    paginationHtml += '</div>';
                ...
                window.deleteMembershipType = function (id) {
                    Swal.fire({
                        title: 'Are you sure?',
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                ...
                                success: function (response) {
                                    Swal.fire({
                                        title: 'Deleted!',
                                        text: 'Membership type has been deleted successfully.',
                                        icon: 'success',
                                        background: '#101966',
                                        color: '#fff'
                                    });
                                ...
                                error: function() {
                                    Swal.fire({
                                        title: 'Error!',
                                        text: 'Something went wrong while deleting the membership type.',
                                        icon: 'error',
                                        background: '#101966',
                                        color: '#fff'
                                    });
                ...
                window.fetchMembershipTypes = fetchMembershipTypes;
            });
        </script>
    </x-slot>
</x-app-layout>