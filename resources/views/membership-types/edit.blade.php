        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('membership-types.update', $membershipType->id) }}" method="post">
                        @csrf
                        <div>
                            <label for="type_name" class="text-sm font-medium">Type Name</label>
                            <div class="my-3">    
                                <input value="{{ old('type_name', $membershipType->type_name) }}" name="type_name" placeholder="Enter membership type" type="text" class="border-gray-300 shadow-sm w-1/2 rounded-lg">
                                @error('type_name')
