<x-app-layout>
    <x-slot name="header">
        <x-page-header title="{{ __('Bureaus') }}">
            <x-slot name="createButton">
                <x-create-button 
                    :route="route('bureaus.create')" 
                    permission="create bureaus" />
            </x-slot>
        </x-page-header>
    </x-slot>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <div class="relative w-48">
                                ...
                                <input type="text" id="searchInput" 
                                    placeholder="Search Bureau Name" class="pl-8 pr-2 py-2 border border-gray-300 dark:border-gray-600 
                    ...
                    <div class="sm:hidden space-y-3 mb-4">
                        <div class="relative w-full">
                            ...
                            <input type="text" id="mobileSearchInput" placeholder="Search Bureau Name" 
                            ...
                            <table id="bureausTable" class="w-full bg-white dark:bg-gray-900 text-sm">
                                <thead class="bg-[#101966] dark:bg-gray-700 text-gray-200 dark:text-gray-200">
                                    <tr class="border-b dark:border-gray-700">
                                        ...
                                        <th class="px-6 py-3 text-center font-medium border-l dark:border-gray-700 border-white column-bureau_name">Bureau Name</th>
        ...
        <script>
            $(document).ready(function () {
                ...
                $('#perPage, #mobilePerPage').on('change', function () {
                    fetchBureaus(1, $('#searchInput').val() || $('#mobileSearchInput').val(), $(this).val());
                });

                $('#sortBy, #mobileSortBy').on('change', function() {
                    fetchBureaus(1, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val());
                });


                function fetchBureaus(page = 1, search = '', perPage = $('#perPage').val() || $('#mobilePerPage').val()) {
                    ...
                    bureaus.forEach((bureau, index) => {
                        ...
                        let row = `
                            <tr class="border-b table-row-hover table-row-animate dark:border-gray-700">
                                ...
                                <td class="px-6 py-4 text-center column-bureau_name">${bureau.bureau_name}</td>
                                <td class="px-6 py-4 text-center column-created">${bureau.created_at}</td>
                                <td class="px-6 py-4 text-center">
                                    <div class="flex justify-center items-center space-x-2">
                                        ${bureau.can_edit ? `
                                        <a href="/bureaus/${bureau.id}/edit" 
                                        ...
                                        ` : ''}
                                        ${bureau.can_delete ? `
                                        <button onclick="deleteBureau(${bureau.id})" 
                ...
                function renderPagination(data) {
                    ...
                    if (data.current_page > 1) {
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchBureaus(1, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                &laquo; First
                            </button>
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchBureaus(${data.current_page - 1}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                Previous
                            </button>`;
                    }
                    ...
                    for (let i = startPage; i <= endPage; i++) {
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border ${i === currentPage ? 'bg-[#101966] text-white' : 'bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white'}"
                                onclick="fetchBureaus(${i}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                ${i}
                            </button>`;
                    }
                    if (endPage < totalPages) {
                        ...
                        paginationHtml += `
                            <button class="px-3 py-1 rounded border ${totalPages === currentPage ? 'bg-[#101966] text-white' : 'bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white'}"
                                onclick="fetchBureaus(${totalPages}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                ${totalPages}
                            </button>`;
                    }
                    if (data.current_page < data.last_page) {
                        paginationHtml += `
                            ...
                            <button class="px-3 py-1 rounded border bg-white hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
                                onclick="fetchBureaus(${data.last_page}, $('#searchInput').val() || $('#mobileSearchInput').val(), $('#perPage').val() || $('#mobilePerPage').val())">
                                Last &raquo;
                            </button>`;
                    }
                    paginationHtml += '</div>';
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                ...
                                success: function (response) {
                                    Swal.fire({
                                        title: 'Deleted!',
                                        text: 'Bureau has been deleted successfully.',
                                        icon: 'success',
                                        background: '#101966',
                                        color: 'white'
                                    });
                                ...
                                error: function() {
                                    Swal.fire({
                                        title: 'Error!',
                                        text: 'Something went wrong while deleting the bureau.',
                                        icon: 'error',
                                        background: '#101966',
                                        color: 'white'
                                    });
